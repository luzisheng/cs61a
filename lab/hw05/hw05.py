class VendingMachine:
    ...
    def vend(self):
        ...
        return 'Here is your {} and ${} change.'.format(self.item, self.funds)
        self.funds = 0
...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        if len(t.branches) == 1 or (len(t.branches) == 2 and t.branches[0].label <= t.label < t.branches[1].label):
...
def preorder(t):
    ...
    order = []
    def helper(t):
        nonlocal order # nonlocal is a good habit!
        order.append(t.label)
        if not t.is_leaf():
            for _ in map(helper, t.branches):
                pass
    helper(t)
    return order
