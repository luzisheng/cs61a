...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        distance = 0
        while not inspect_place == beehive:
            if inspect_place.bees != [] and self.min_range <= distance <= self.max_range:
                return rANTdom_else_none(inspect_place.bees)
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.first_queen_exists:
            self.is_first_queen = False
        else:
            QueenAnt.first_queen_exists = True
    ...
    def action(self, gamestate):
        ...
        if not self.is_first_queen:
            ...
            while not behind_place == None:
                if behind_place.ant and behind_place.ant not in self.buffed_ants:
                    behind_place.ant.damage *= 2
                    self.buffed_ants.append(behind_place.ant)
    ...
    def reduce_armor(self, amount):
        ...
        if self.is_first_queen and self.armor <= 0:
            bees_win()
        # END Problem EC
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 5
        return self.base_damage - 0.2*distance - 0.05*self.insects_shot
        # END Problem Optional 5
